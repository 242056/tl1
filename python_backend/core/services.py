...
class DialogService:
    ...
    def _contextual_unknown_reply(self, s: SessionContext) -> Dict[str, Any]:
        ...
        return {
            ...
            "intent": "UNKNOWN",
            "reply": (
                "Я помогу с четырьмя задачами: бэктест портфеля, стресс-сценарий, "
                "математический паспорт продукта (Анти-Гугл) или ИИ-черновик для ручной коррекции портфеля. "
                "Что вас интересует?"
            ),
            "actions": ["Бэктест", "Сценарий", "Математический паспорт", "ИИ-черновик"],
            "context": s.model_dump(),
        }
